...
function answer = checkAnswer5(inputString);
    ...
    % if answer is "A" "a"
    if length(regexprep(answer,"[ABCDE]","")) > 0 || length(answer) == 0
        disp("")
        disp("Your answer should be A, B, C, D, or E.")
        disp("")
        inputString = input("Enter your answer again: ","s");
        disp("")
        answer = strsplit(strtrim(inputString), " ");
        ...
        answer = toupper(answer);
        if length(regexprep(answer,"[ABCDE]","")) > 0
            disp("")
