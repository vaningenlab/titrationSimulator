...
function peakInfo(peakNumber)
    ...
    global wHv wNv dwHv dwNv koff ppmaxis wHvppm wNvppm dwHvppm dwNvppm centerHppm centerNppm my_pi aa_string
    ...
    if nargin == 0
        ...
        disp("")
        if ppmaxis == 0
            printf("\tposition free state  (H/N, Hz): %6.1f / %6.1f\n", wHv(p)/(2*my_pi), wNv(p)/(2*my_pi));
            printf("\tposition bound state (H/N, Hz): %6.1f / %6.1f\n", (wHv(p)+dwHv(p))/(2*my_pi), (wNv(p)+dwNv(p))/(2*my_pi));
            printf("\tchemical shift perturbation   : %6.1f / %6.1f\n", dwHv(p)/(2*my_pi), dwNv(p)/(2*my_pi));
